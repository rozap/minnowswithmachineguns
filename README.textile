...
A utility for arming (creating) many minnows (digital ocean instances) to attack (load test) targets (web applications).
...
h2. Credits
Minnows with machine guns is 99% code from "Bees with Machine Guns":http://github.com/newsapps/beeswithmachineguns but with some changes to make it work for Digital Ocean rather than AWS. All credit goes to them. 
